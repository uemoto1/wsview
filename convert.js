...
template_from_cif = {
"dft": `! This file is automatically generated by Web SALMON View
&calculation
    !type of theory
    theory = 'dft'
/
...
/
`,
"tddft_pulse": `! This file is automatically generated by Web SALMON View
&calculation
    !type of theory
    theory = 'tddft_pulse'
/
...
function generateInput(code, mode, dgrid, unit) {
    const data = readCIF(code)

    const length_factor = (unit == 'au') ? (1.0 / 0.52917721067) : 1.0;
    const time_factor = (unit == 'au') ? (1.0 / 0.02418884326505) : 1.0;
    const energy_factor = (unit == 'au') ? (1.0 / 27.21138505) : 1.0;
    ...
    if ((alpha == 90) && (beta == 90) && (gamma == 90)) {
        lattice_info += indent + "al(1:3) = "
        lattice_info += + (a * length_factor).toFixed(6) + "d0, ";
        lattice_info += + (b * length_factor).toFixed(6) + "d0, ";
        lattice_info += + (c * length_factor).toFixed(6) + "d0";
    } else {
        ...
        lattice_info += indent + "al_vec1(1:3) = ";
        lattice_info += (ax * length_factor).toFixed(6) + "d0, ";
        lattice_info += (ay * length_factor).toFixed(6) + "d0, ";
        lattice_info += (az * length_factor).toFixed(6) + "d0\n";
        lattice_info += indent + "al_vec2(1:3) = ";
        lattice_info += (bx * length_factor).toFixed(6) + "d0, ";
        lattice_info += (by * length_factor).toFixed(6) + "d0, ";
        lattice_info += (bz * length_factor).toFixed(6) + "d0\n";
        lattice_info += indent + "al_vec3(1:3) = ";
        lattice_info += (cx * length_factor).toFixed(6) + "d0, ";
        lattice_info += (cy * length_factor).toFixed(6) + "d0, ";
        lattice_info += (cz * length_factor).toFixed(6) + "d0\n";
    }
    ...
    const nx = Math.ceil(a/dgrid/2) * 2;
    const ny = Math.ceil(b/dgrid/2) * 2;
    const nz = Math.ceil(c/dgrid/2) * 2;
    const dx = a / nx;
    const dy = b / ny;
    const dz = c / nz;
    const dmin = Math.min(dx, dy, dz)
    const dt = 0.02 * dmin * dmin;
    
    var tmp = template_from_cif[mode];
    tmp = tmp.replace("%SYSNAME%", data._chemical_formula_structural);
    tmp = tmp.replace("%UNIT%", unit);
    ...
    tmp = tmp.replace("%NX%", nx);
    tmp = tmp.replace("%NY%", ny);
    tmp = tmp.replace("%NZ%", nz);
    tmp = tmp.replace("%DT%", ((dt * time_factor).toFixed(1-Math.floor(Math.log10(dt * time_factor)))) + "d0");
    tmp = tmp.replace("%NT%", (Math.ceil(10.0 / dt / 1000 + 1) * 1000));
    tmp = tmp.replace("%TW1%", (10.0 * time_factor).toFixed(6)+"d0");
    tmp = tmp.replace("%OMEGA1%", (1.55 * energy_factor).toFixed(6)+"d0");
    return tmp;
}
